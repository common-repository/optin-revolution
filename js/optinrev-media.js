  ...
  action_add_image: function( img ) {     
     ...
     jQuery.post(ajaxurl, {action : 'optinrev_action', optinrev_add_image_briefcase : img, optinrev_curr_page : 'optinrevolution/optin1'}, function(){wtfnm.msg( img + '_msg', 'Successfully added.' );});
     }
     return false;
  },
  action_del_image: function( img ) {        
   if ( confirm('Are you sure, you want to remove this image ?') ) {
   jQuery.post(ajaxurl, {action : 'optinrev_action', optinrev_del_image_briefcase : img, optinrev_curr_page : 'optinrevolution/optin1'}, function(){wtfnm.msg( img + '_msg', 'Successfully removed.' );});   
   }
   return false; 
  },
  action_update_button: function( img, src ) {       
   if ( confirm('Are you sure, you want to update the action button of Optin Popup 1') ) {   
   jQuery.post(ajaxurl, {action : 'optinrev_action', optinrev_add_button_briefcase : src, optinrev_curr_page : 'optinrevolution/optin1'}, function(){wtfnm.msg( img + '_msg', 'Successfully updated.' );});   
   }   
   return false; 
  }    
 }
