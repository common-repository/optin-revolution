 ...
 */
 ...
 if ( !function_exists('optinrev_action_callback') )
 {
     function optinrev_action_callback()
     {
      ...
      if ( !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) !== 'xmlhttprequest' ) die('Invalid request.');
      if ( !substr_count( $_SERVER['REQUEST_URI'], 'admin-ajax.php' ) ) die('Invalid request.');//keep silent
      ...
      if (isset( $_POST['save_setup_settings'] ))
      {
          ...
          $_POST['save_setup_settings'] = urldecode($_POST['save_setup_settings']);
...
if ( $optin = optinrev_get( 'optinrevolution/optin1' ) ) {
  $optin = maybe_unserialize( $optin );
  } else {
  $optin = unserialize(optinrev_get('optinrev_default'));  
}

if ( empty($optin) ) return false;

$optin = array_map( 'optinrev_map_decode_entities', $optin );
...
echo optinrev_cssminify($modal);
 ...
 if ( !function_exists('optinrev_popup_vars') ) {
  ...
  $content = ( isset( $optin['optinrev_data'] ) ) ? urldecode(urldecode(optinrev_decodecontent( $optin['optinrev_data'] ))) : '';
  $content = preg_replace("/'/","\'",preg_replace('/\s+/', ' ', $content ));
  ...
  {         
      ...
      if(!empty($custom_fields)){
          foreach($custom_fields as $row){                        
              if ( isset( $_POST['wysija']['field']['cf_'. $row->id] ) && $cf = esc_html($_POST['wysija']['field']['cf_'. $row->id])  ) {
                  $cf_fields[ 'cf_' . $row->id ] = $cf;
              }
          }
      }
      ...
      $user_id = $userHelper->addSubscriber($data, true);    
    
      if ( is_numeric( $user_id ) && count($cf_fields) > 0 ) {
      	WJ_FieldHandler::handle_all( $cf_fields, $user_id	);
      }
  ...
  function optinrev_map_entities( $str ) {
  return ( !is_array( $str ) ) ? htmlentities( $str ) : $str;
  }

  function optinrev_map_decode_entities( $str ) {
  return ( !is_array( $str ) ) ? html_entity_decode( stripcslashes(urldecode($str)) ) : $str;
  }

  function optinrev_decodecontent( $content ) {
  return (base64_decode($content, true)) ? stripcslashes( base64_decode( $content ) ) : $content;
  }
  
  function optinrev_get_optin() {
  if ( $optin = optinrev_get( 'optinrevolution/optin1' ) ) {  
  return array_map( 'optinrev_map_decode_entities', maybe_unserialize( $optin ) );
  }
  
  return array_map( 'optinrev_map_decode_entities', unserialize(optinrev_get('optinrev_default')));
  }  
