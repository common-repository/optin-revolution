...
define( 'OPTINREV_XMLRPC_URL', 'http://www.optinrevolution.com/xmlrpc/' );
...
function optinrev_js()
{
  ...
  <?php }
  else if ( urldecode($plugin_page) == 'optinrevolution/optin1' ) {

  wp_enqueue_script( 'optinrevjsv', home_url( sprintf('?optinrev-edit-init-js=%s&t=', urldecode($plugin_page), optinrev_cid()) ), array(), $wp_version, true );
...
<div id="optin-revolution">
  <div class="wrap fform pro-license">
    ...
    <form id="cred_form" name="cred_form" method="post" action="<?php echo $this_uri;?>">
      ...
      <div class="row"><label>&nbsp;</label><input type="submit" name="Submit" class="button button-default optinrev_xbutton" value="<?php _e('Save', 'optin'); ?>" />&nbsp;<span id="acn_verify"></span></div>
...
<div class="icon32" id="icon-options-general"><br /></div><?php echo "<h2>" . __( 'Optin Revolution Settings', 'optinrev_trdom' ) . "</h2>"; ?> <br />
  ...
  <form name="optinrev_form" id="optinrev_form" method="post" action="<?php echo $this_uri;?>">
    ...
    <div class="row" id="save_showset"><label>&nbsp;</label><span class="submit"><input type="button" name="optinrev_save_showset" class="button button-default optinrev_xbutton" id="optinrev_save_showset" value="Save"/></span></div>
    <br />    
    <div class="row"><label class="lbladmin"><?php _e('Cookies'); ?></label><input type="button" class="button button-default optinrev_xbutton" name="optinrev_clean_cookies" value="Clear Cookies" onclick="admin.cookies();"/></div>
    <div class="row"><label class="lbladmin"><?php _e('Email Form Settings'); ?></label><input type="button" class="button button-default optinrev_xbutton" name="optinrev_reset_emailforms" value="Reset" onclick="wtfn.reset_emailform();"/></div>
    <div class="row"><label class="lbladmin"><?php _e('Popup and Email Form Settings'); ?></label><input type="button" class="button button-default optinrev_xbutton" name="optinrev_reset_popup" value="Reset" onclick="wtfn.reset_popup();"/></div>    
  </form>
  </div>
...
function optinrev_setup() {
  ...
  //Get optin  
  $_popup = optinrev_get_optin();
  $content = ( isset( $_popup['optinrev_excerpt'] ) ) ? urldecode(urldecode(optinrev_decodecontent( $_popup['optinrev_excerpt'] ))) : '';

  $is_call_action = (isset( $_popup['optinrev_call_action_button'])) ? $_popup['optinrev_call_action_button'] : 'get_access1';
  $is_close_btn = (isset( $_popup['optinrev_close_popup_image'])) ? $_popup['optinrev_close_popup_image'] : 'close1';
  ...
  $email_form_opt = (isset( $_popup['optinrev_email_form_opt'])) ? $_popup['optinrev_email_form_opt'] : 'virtualsender';
  
  $foptin = (isset( $_popup['optinrev_foptin_active'])) ? $_popup['optinrev_foptin_active'] : 'aweber';
  $foptin_form = (isset( $_popup['optinrev_foptin_form_active'])) ? $_popup['optinrev_foptin_form_active'] : '';
  
  $femail_validate = (isset( $_popup['optinrev_femail_validate'])) ? $_popup['optinrev_femail_validate'] : '';
  
  $wylist_id = (isset( $_popup['wysija_list_id'])) ? $_popup['wysija_list_id'] : ''; 
  
  //input text
  $inputs = (isset($_popup['optinrev_email_form'])) ? $_popup['optinrev_email_form'] : '';
  $inputs_enabled = (isset($_popup['optinrev_input_enabled'])) ? $_popup['optinrev_input_enabled'] : '';
  
  //goto website button location
  $gw = (isset( $_popup['optinrev_gotowebsite'])) ? $_popup['optinrev_gotowebsite'] : 'top';
  
  $optinrev_delay = (isset( $_popup['optinrev_delay'])) ? $_popup['optinrev_delay'] : 0;
  $optinrev_ctcurl = (isset($_popup['optinrev_ctcurl'])) ? $_popup['optinrev_ctcurl'] : '';
...
<div class="icon32" id="icon-options-general"><br /></div><?php echo "<h2>" . __( $optin[ $plugin_page ] . ' Settings', 'wotp_trdom' ) . "</h2>"; ?> <br />
  ...
  <form name="optinrev_setup_form" id="optinrev_setup_form" method="POST" action="admin-ajax.php">
    ...
    <input type="hidden" name="optinrev_close_button" id="optinrev_close_button" value="<?php echo $_popup['optinrev_close_button'];?>"/>
    <input type="hidden" name="optinrev_close_button_class" id="optinrev_close_button_class"/>
    <input type="hidden" name="optinrev_dragging" id="optinrev_dragging" value="<?php echo $_popup['optinrev_dragging'];?>"/>
    ...
    <input type="button" name="action_reset" class="button button-default optinrev_xbutton" id="action_reset" value="<?php _e('Reset') ?>" />        
    </div>    
    ...
    <textarea id="optinrev_excerpt_content"><?php echo htmlentities($content, ENT_QUOTES | ENT_IGNORE, "UTF-8");?></textarea><br />
    </div>
    ...
    <div class="optinrev_mail_providers">
          ...
          <div class="box2">
              ...
              <textarea name="optinrev_optin_<?php echo $k;?>" id="optinrev_optincode_<?php echo $k;?>" cols="100" rows="12"><?php echo ( isset( $_popup['optinrev_optin_'. $k] ) ) ? htmlspecialchars($_popup['optinrev_optin_'. $k]) : '';?></textarea>
              <div class="row"><input type="button" name="save_wait" class="button button-default optinrev_xbutton" value="Optin Code Update" onclick="wtfn.optin_form('<?php echo $k;?>');"></div>
              ...
              <?php
                ...
                $wy_cf = ( isset($_popup['wysija_custom_fields']) ) ? $_popup['wysija_custom_fields'] : '';
                $wy_cf_val = ( isset($_popup['wysija_cf_validate']) ) ? $_popup['wysija_cf_validate'] : '';
                ...
                //Custom fields
                $htm .= '<h3>MailPoet Custom Fields</h3>';          
                $WJ_Field = new WJ_Field();
                $custom_fields = $WJ_Field->get_all();
                $flds = array();
                
                $htm .= '<p><select name="wysija_custom_fields" id="wysija_custom_fields">';          
                $htm .= '<option value="">--Select Custom Fields--</option>';
                if( !empty($custom_fields) ) {          
                  foreach($custom_fields as $row){
                    $htm .= '<option value="cf_'. $row->id .'">'. ucfirst($row->name) ."</option>";
                    $flds['cf_' . $row->id] = ucfirst($row->name);
                  }            
                }
                $htm .= '</select>&nbsp;<a href="javascript:void(0);" title="Add custom field." onclick="wtfn.mailpoet_add_cfields()">[+]</a></p>';
                
                //Active custom fields
                $ac_cf_htm = '';
                if ( isset($_popup['wysija_cf_active']) && count($_popup['wysija_cf_active']) > 0 && $ac_cf = $_popup['wysija_cf_active'] ) {          
                    foreach( $ac_cf as $acfk => $acfv ) {                
                      $cf_val = ( isset( $wy_cf_val[$acfk] ) ) ? 'checked' : ''; 
                      $ac_cf_htm .= '<div><label>'. $flds[$acfk] .'</label>&nbsp;<input type="text" name="wysija_cf_active['. $acfk .'][]" id="'. $acfk .'" value="'. $acfv[0] .'" onblur="wtfn.mailpoet_stage_inputtext(this);" class="wysija_cf_active" placeholder="'. $acfv[0] .'">&nbsp;<a href="javascript:void(0);" title="Remove custom field." data-id="'. $acfk .'" onclick="wtfn.mailpoet_remove_cfields(this);">[-]</a></div>';      
                    }
                }
                          
                echo $htm .= '<div class="clear"></div><p><div id="wysija_cf_list">'. $ac_cf_htm .'</div></p><br />';
              ...
              <?php
                //email value;
                $optinrev_email = (isset( $_popup['optinrev_email'])) ? $_popup['optinrev_email'] : 'Enter Your Email...';
              ?>

              <div class="foptin_ifo" <?php echo ( $foptin_form )? 'style="display:block"':'';?>>
                <h3>Your <span id="foptin_ifo_lbl"><b><?php echo $mail_form[$foptin];?></b></span> form code is now linked to <br />Optin Revolution Lite</h3><br /><br /><br />
    ...
    <div class="fbox"><input type="text" name="optinrev_inputh" id="optinrev_inputh" value="<?php echo (isset( $_popup['optinrev_inputh']))?$_popup['optinrev_inputh']:'50';?>" size="10" readonly>px</div>
    <div class="wjui-box"><div id="optinrev_inpuths" class="wjui"></div></div>
    <div class="clear"></div>
    </div>
    <div class="row"><label><?php _e('Width'); ?></label>
    <div class="fbox"><input type="text" name="optinrev_inputw" id="optinrev_inputw" value="<?php echo (isset( $_popup['optinrev_inputw']))?$_popup['optinrev_inputw']:'160';?>" size="10" readonly>px</div>
    <div class="wjui-box"><div id="optinrev_inputws" class="wjui"></div></div>
    <div class="clear"></div>
    </div>

    <div class="row"><label><?php _e('Text Color'); ?></label><input type="text" name="optinrev_inputtc" class="color {hash:true}" value="#<?php echo (isset( $_popup['optinrev_inputtc']))?$_popup['optinrev_inputtc']:'000000';?>" size="10"></div>

    <div class="row"><label><?php _e('Font Size'); ?></label>
    <div class="fbox"><input type="text" name="optinrev_inputfz" id="optinrev_inputfz" value="<?php echo (isset( $_popup['optinrev_inputfz']))?$_popup['optinrev_inputfz']:'12';?>" size="10" readonly>px</div>
    <div class="wjui-box"><div id="optinrev_inputfzs" class="wjui"></div></div>
    <div class="clear"></div>
    </div>

    <div class="row"><label><?php _e('Background Color'); ?></label><input type="text" name="optinrev_inputc" class="color {hash:true}" value="#<?php echo (isset( $_popup['optinrev_inputc']))?$_popup['optinrev_inputc']:'ffffff';?>" size="10"></div>
    <div class="row"><label><?php _e('Border Color'); ?></label><input type="text" name="optinrev_inputb" class="color {hash:true}" value="#<?php echo (isset( $_popup['optinrev_inputb']))?$_popup['optinrev_inputb']:'000000';?>" size="10"></div>

    <div class="row"><label><?php _e('Border Thickness'); ?></label>
    <div class="fbox"><input type="text" name="optinrev_inputbt" id="optinrev_inputbt" value="<?php echo (isset( $_popup['optinrev_inputbt']))?$_popup['optinrev_inputbt']:'1';?>" size="10" readonly>px</div>
    ...
    <div class="row"><label><?php _e('Background Color'); ?></label><input type="text" name="optinrev_wbg_color" id="optinrev_wbg_color" class="color {hash:true}" value="<?php echo $_popup['optinrev_wbg_color'];?>" size="10"></div>
    <div class="row"><label><?php _e('Background Opacity'); ?></label>
    <div class="fbox"><input type="text" name="optinrev_wbg_opacity" id="optinrev_wbg_opacity" value="<?php echo (isset($_popup['optinrev_wbg_opacity']))?$_popup['optinrev_wbg_opacity']:'0';?>" size="10" readonly>%</div>
    ...
    <div class="fbox"><input type="text" name="optinrev_delay" id="optinrev_vdelay" value="<?php echo (isset( $_popup['optinrev_delay']))?$_popup['optinrev_delay']:'0';?>" size="10" readonly>Sec</div>
    ...
    <div class="row"><label><?php _e('Background Color'); ?></label><input type="text" name="optinrev_pwbg_color" class="color {hash:true}" value="<?php echo (isset( $_popup['optinrev_pwbg_color']))?$_popup['optinrev_pwbg_color']:'ffffff';?>" size="10"></div>
    <div class="row"><label><?php _e('Border Color'); ?></label><input type="text" name="optinrev_border_color" class="color {hash:true}" value="<?php echo (isset( $_popup['optinrev_border_color']))?$_popup['optinrev_border_color']:'000000';?>" size="10"></div>

    <div class="row"><label><?php _e('Border Thickness'); ?></label>
    <div class="fbox"><input type="text" name="optinrev_border_thickness" id="optinrev_vborder_thickness" value="<?php echo (isset( $_popup['optinrev_border_thickness']))?$_popup['optinrev_border_thickness']:'1';?>" size="10" readonly>px</div>
    <div class="wjui-box"><div id="optinrev_sborder_thickness" class="wjui"></div></div>
    <div class="clear"></div>
    </div>

    <div class="row"><label><?php _e('Border Opacity'); ?></label>
    <div class="fbox"><input type="text" name="optinrev_border_opacity" id="optinrev_border_opacity" value="<?php echo (isset( $_popup['optinrev_border_opacity']))?$_popup['optinrev_border_opacity']:'0';?>" size="10" readonly>%</div>
    <div class="wjui-box"><div id="border_opacity_slider" class="wjui"></div></div>
    <div class="clear"></div>
    </div>

    <?php if ( isset( $_popup['optinrev_round_border']) == 'on' ) { ?>
    <div class="row" id="_nbr"><label class="lbladmin"><?php _e('Border Radius'); ?></label>
    <div class="fbox"><input type="text" name="optinrev_border_radius" id="optinrev_border_radius" value="<?php echo (isset( $_popup['optinrev_border_radius']))?$_popup['optinrev_border_radius']:'0';?>" size="10" readonly>%</div>
    <div class="wjui-box"><div id="border_radius_slider" class="wjui"></div></div>
    <div class="clear"></div>
    </div>
    <?php }?>

    <div><label class="lbladmin"><?php _e('Rounded Border'); ?></label><div class="fbox"><input type="checkbox" name="optinrev_round_border" id="optinrev_round_border" <?php echo (isset( $_popup['optinrev_round_border'])=='on') ? 'checked':'';?>/></div><div class="clear"></div></div>
    <div class="row"><label><?php _e('Popup Top Margin'); ?></label>
    <div class="fbox"><input type="text" name="optinrev_top_margin" id="optinrev_vtop_margin" value="<?php echo (isset( $_popup['optinrev_top_margin']))?$_popup['optinrev_top_margin']:'0';?>" size="10" readonly>px</div>
    <div class="wjui-box"><div id="optinrev_stop_margin" class="wjui"></div></div>
    <div class="clear"></div>
    </div>
    <div class="row"><label><?php _e('Popup Width'); ?></label>
    <div class="fbox"><input type="text" name="optinrev_wwidth" id="optinrev_vwidth" value="<?php echo (isset( $_popup['optinrev_wwidth']))?$_popup['optinrev_wwidth']:'900';?>" size="10" readonly>px</div>
    ...
    <div class="fbox"><input type="text" name="optinrev_hheight" id="optinrev_vheight" value="<?php echo (isset( $_popup['optinrev_hheight']))?$_popup['optinrev_hheight']:'600';?>" size="10" readonly>px</div>
    ...
    <div class="row"><label><?php _e('Hyperlink Color'); ?></label><input type="text" name="optinrev_link_color" id="optinrev_link_color" class="color {hash:true}" value="<?php echo (isset( $_popup['optinrev_link_color']))?$_popup['optinrev_link_color']:'1122CC';?>" size="10">&nbsp;<span>Link, visited, hover and active</span></div>
    <div><label class="lbladmin"><?php _e('Hyperlink Underline'); ?></label><div class="fbox"><input type="checkbox" name="optinrev_link_underline" id="optinrev_link_underline" <?php echo (isset( $_popup['optinrev_link_underline'])=='on') ? 'checked':'';?>/></div><div class="clear"></div></div>
